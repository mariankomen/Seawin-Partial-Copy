trigger AfterOpportunityTrigger on Opportunity (after update, before delete) {
	if(Trigger.isAfter && Trigger.isUpdate){
        ...
        i++;
        i++;
        if(!Test.isRunningTest()) OpportunityDashboardHandler.updateExistingRecords(Trigger.new);
    } else if(Trigger.isBefore && Trigger.isDelete){
        if(!Test.isRunningTest()) OpportunityDashboardHandler.deleteRecords(Trigger.old);
    }
}